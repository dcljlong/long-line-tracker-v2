      ...
      <div className={`px-4 sm:px-6 py-3 sm:py-4 ${UI.card} ${UI.cardPad}`}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-start gap-3 sm:gap-4 min-w-0">
            <button
              onClick={onBack}
              className="p-2 hover:bg-slate-900/30 rounded-lg transition-colors text-slate-400 hover:text-slate-300 shrink-0"
              aria-label="Back"
            >
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs font-mono text-slate-400 bg-slate-900/30 px-2 py-0.5 rounded">{eq.asset_id}</span>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                ...
                className={`flex-1 sm:flex-none p-2.5 border rounded-lg transition-colors ${
                  showQR ? 'bg-amber-500/90 text-slate-950 border-amber-400/30' : 'border-white/10 text-slate-300 hover:bg-slate-900/30'
                }`}
                title="Show QR Code"
              ...
              {isAdmin && (
                <button
                  onClick={onEdit}
                  className="flex-1 sm:flex-none px-4 py-2.5 border border-white/10 rounded-lg text-sm font-medium text-slate-200 hover:bg-slate-900/30 transition-colors flex items-center justify-center gap-2"
                >
        ...
        {showQR && (
          <div className={`${UI.card} ${UI.cardPad}`}>
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <div className="bg-slate-900/30 p-3 rounded-2xl ring-1 ring-white/10">
                <img
                  src={qrImageUrl}
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-xs text-slate-400 mb-1">Scan this code to quickly access this equipment's detail page.</p>
                <p className="text-xs font-mono text-slate-400 bg-slate-900/30 inline-block px-2 py-1 rounded mb-4">{eq.qr_code}</p>
                <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-amber-500/90 text-slate-950 rounded-lg text-xs font-medium hover:bg-amber-500 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 border border-white/10 rounded-lg text-xs font-medium text-slate-200 hover:bg-slate-900/30 transition-colors"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className={`${UI.card} p-5`}>
              ...
              <dl className="space-y-3">
                <div>
                  <dt className="text-xs text-slate-400 uppercase tracking-wider">Category</dt>
                  <dd className="text-sm text-white mt-0.5">{eq.category}</dd>
                </div>
                <div>
                  <dt className="text-xs text-slate-400 uppercase tracking-wider">Condition</dt>
                  <dd className="mt-0.5"><ConditionBadge condition={eq.condition} /></dd>
                </div>
                <div>
                  <dt className="text-xs text-slate-400 uppercase tracking-wider">QR Code</dt>
                  <dd className="text-sm text-white font-mono mt-0.5">{eq.qr_code}</dd>
                </div>
                {eq.notes && (
                  <div>
                    <dt className="text-xs text-slate-400 uppercase tracking-wider">Notes</dt>
                    <dd className="text-sm text-slate-300 mt-0.5">{eq.notes}</dd>
                  </div>
                )}
                <div>
            ...
            <div className={`${UI.card} p-5`}>
              ...
              <div className={`p-4 rounded-lg mb-4 ${
                ...
                tagState === 'Expired' ? 'bg-red-500/10 ring-1 ring-red-500/20' : 'bg-slate-900/30 ring-1 ring-white/10'
              }`}>
              ...
              <dl className="space-y-3">
                <div>
                  <dt className="text-xs text-slate-400 uppercase tracking-wider">Last Test</dt>
                ...
                <div>
                  <dt className="text-xs text-slate-400 uppercase tracking-wider">Next Due</dt>
                  <dd className="text-sm text-white mt-0.5">
                    {nextDue
                ...
                <div>
                  <dt className="text-xs text-slate-400 uppercase tracking-wider">Threshold</dt>
                  <dd className="text-sm text-slate-300 mt-0.5">{eq.tag_threshold_days} days</dd>
                </div>
              </dl>
            </div>
            ...
            {(status === 'In Use' || status === 'Overdue') && (
              <div className={`${UI.card} p-5`}>
                ...
                <dl className="space-y-3">
                  <div>
                    <dt className="text-xs text-slate-400 uppercase tracking-wider">Assigned To</dt>
                    <dd className="text-sm text-white font-medium mt-0.5">{eq.assigned_to}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-slate-400 uppercase tracking-wider">Site</dt>
                    <dd className="text-sm text-white mt-0.5">{eq.assigned_site}</dd>
                  </div>
                  {eq.assigned_job && (
                    <div>
                      <dt className="text-xs text-slate-400 uppercase tracking-wider">Job Reference</dt>
                      <dd className="text-sm text-white font-mono mt-0.5">{eq.assigned_job}</dd>
                    </div>
                  )}
                  {eq.expected_return_date && (
                    <div>
                      <dt className="text-xs text-slate-400 uppercase tracking-wider">Expected Return</dt>
          ...
          <div className="lg:col-span-2">
            <div className={`${UI.card}`}>
              <div className={`px-5 py-4 border-b ${UI.divider} flex items-center justify-between`}>
                ...
                <span className="text-xs text-slate-400">{selectedMovements.length} entries</span>
              </div>
                      ...
                      {selectedMovements.map((m) => (
                        <div key={m.id} className="relative pl-10">
                          ...
                          <div className="bg-slate-800/40 rounded-xl p-4 ring-1 ring-white/5">
                            ...
                            {m.notes && (
                              <p className="text-sm text-slate-300 mt-2 pt-2 border-t border-white/10">{m.notes}</p>
                            )}
