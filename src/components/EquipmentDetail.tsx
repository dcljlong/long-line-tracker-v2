          ...
          <div className="bg-white rounded-xl border border-gray-100 p-6">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center sm:text-left">
                <h4 className="text-sm font-semibold text-gray-900 mb-1">Equipment QR Code</h4>
                <p className="text-xs text-gray-500 mb-1">Scan this code to quickly access this equipment's detail page.</p>
                <p className="text-xs font-mono text-gray-400 bg-gray-50 inline-block px-2 py-1 rounded mb-4">{eq.qr_code}</p>
                <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-[#1e3a5f] text-white rounded-lg text-xs font-medium hover:bg-[#2d5a8e] transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-lg text-xs font-medium text-gray-700 hover:bg-gray-50 transition-colors"
                  >
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h3 className="text-sm font-semibold text-gray-900 mb-4">Equipment Details</h3>
              <dl className="space-y-3">
                <div>
                  <dt className="text-xs text-gray-400 uppercase tracking-wider">Category</dt>
                  <dd className="text-sm text-gray-900 mt-0.5">{eq.category}</dd>
                </div>
                <div>
                  <dt className="text-xs text-gray-400 uppercase tracking-wider">Condition</dt>
                  <dd className="mt-0.5"><ConditionBadge condition={eq.condition} /></dd>
                </div>
                <div>
                  <dt className="text-xs text-gray-400 uppercase tracking-wider">QR Code</dt>
                  <dd className="text-sm text-gray-900 font-mono mt-0.5">{eq.qr_code}</dd>
                </div>
                {eq.notes && (
                  <div>
                    <dt className="text-xs text-gray-400 uppercase tracking-wider">Notes</dt>
                    <dd className="text-sm text-gray-600 mt-0.5">{eq.notes}</dd>
                  </div>
                )}
                <div>
            ...
            {/* Compliance */}
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h3 className="text-sm font-semibold text-gray-900 mb-4">Test & Tag Compliance</h3>
              <div className={`p-4 rounded-lg mb-4 ${
                tagState === 'OK' ? 'bg-emerald-50' :
                tagState === 'Due Soon' ? 'bg-amber-50' :
                tagState === 'Expired' ? 'bg-red-50' : 'bg-gray-50'
              }`}>
              ...
              <dl className="space-y-3">
                <div>
                  <dt className="text-xs text-gray-400 uppercase tracking-wider">Last Test</dt>
                ...
                <div>
                  <dt className="text-xs text-gray-400 uppercase tracking-wider">Threshold</dt>
                  <dd className="text-sm text-gray-600 mt-0.5">{eq.tag_threshold_days} days</dd>
                </div>
              </dl>
            </div>
            ...
            {(status === 'In Use' || status === 'Overdue') && (
              <div className="bg-white rounded-xl border border-gray-100 p-5">
                <h3 className="text-sm font-semibold text-gray-900 mb-4">Current Assignment</h3>
                <dl className="space-y-3">
                  <div>
                    <dt className="text-xs text-gray-400 uppercase tracking-wider">Assigned To</dt>
                    <dd className="text-sm text-gray-900 font-medium mt-0.5">{eq.assigned_to}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-400 uppercase tracking-wider">Site</dt>
                    <dd className="text-sm text-gray-900 mt-0.5">{eq.assigned_site}</dd>
                  </div>
                  {eq.assigned_job && (
                    <div>
                      <dt className="text-xs text-gray-400 uppercase tracking-wider">Job Reference</dt>
                      <dd className="text-sm text-gray-900 font-mono mt-0.5">{eq.assigned_job}</dd>
                    </div>
                  )}
                  {eq.expected_return_date && (
                    <div>
                      <dt className="text-xs text-gray-400 uppercase tracking-wider">Expected Return</dt>
                      <dd className={`text-sm font-medium mt-0.5 ${
                        new Date(eq.expected_return_date) < new Date() ? 'text-red-600' : 'text-gray-900'
                      }`}>
                      ...
                      {selectedMovements.map((m) => (
                        <div key={m.id} className="relative pl-10">
                          ...
                          <div className="bg-gray-50 rounded-lg p-4">
                            ...
                            <div className="grid grid-cols-2 gap-3 text-sm">
                              {m.assigned_to && (
                                <div>
                                  ...
                                  <p className="text-gray-900 font-medium">{m.assigned_to}</p>
                                </div>
                              )}
                              {m.site && (
                                <div>
                                  <span className="text-xs text-gray-400">Site</span>
                                  <p className="text-gray-900">{m.site}</p>
                                </div>
                              )}
                              {m.job_reference && (
                                <div>
                                  <span className="text-xs text-gray-400">Job Ref</span>
                                  <p className="text-gray-900 font-mono text-xs">{m.job_reference}</p>
                                </div>
                              )}
                            ...
                            {m.notes && (
                              <p className="text-sm text-gray-600 mt-2 pt-2 border-t border-gray-200">{m.notes}</p>
                            )}
